...
/*   Updated: 2024/09/23 16:28:52 by yohanafi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	**copy_char_to_int_map(t_cub *cub, char **char_map, int i, int j)
{
	...
	while (char_map && char_map[cub->height_m] != NULL)
		cub->height_m++;
	if (cub->height_m > 0)
		cub->width_m = ft_strlen(char_map[0]);
	int_map = (int **)malloc(sizeof(int *) * cub->height_m);
	if (!int_map)
		return (NULL);
	while (++i < cub->height_m)
	{
		int_map[i] = (int *)malloc(sizeof(int) * cub->width_m);
		if (!int_map[i])
		{
			while (--i >= 0)
				free(int_map[i]);
...
{
	cub->posX = cub->start_i;
	cub->posY = cub->start_j;
